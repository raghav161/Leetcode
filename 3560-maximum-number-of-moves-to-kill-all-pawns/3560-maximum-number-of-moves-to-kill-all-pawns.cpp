...
public:
    ...
    int solve(vector<vector<int>>& minDist, int idx, int mask, bool Alice, vector<vector<int>>& dp) {
        if (mask == 0) 
            return 0;
        if (dp[idx][mask] != -1) 
            return dp[idx][mask];
        ...
        for (int i = 1; i < minDist.size(); i++) {
            if (mask & (1 << (i - 1))) {
                ...
                if (Alice) 
                    result = max(result, moves + solve(minDist, i, mask ^ (1 << (i - 1)), !Alice, dp));
                else 
                    result = min(result, moves + solve(minDist, i, mask ^ (1 << (i - 1)), !Alice, dp));
            }
        }
    ...
    int maxMoves(int kx, int ky, vector<vector<int>>& positions) {
        ...
        vector<vector<int>> minDist(positions.size()+1, vector<int>(positions.size()+1));
        for(int i=0;i<pos.size();i++)
        {
        ...
        return solve(minDist, 0, (1 << positions.size()) - 1, Alice, dp);
    }
};
